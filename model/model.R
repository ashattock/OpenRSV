...
#
# Verbose options: 
#  day  - print each day index as we iterate
...
model = function(scenario, seed = NA, fit = NULL, uncert = NULL, do_plot = FALSE, verbose = "day") {
  ...
  states = list(infected   = qc(latent, asym, mild, hosp),
                infectious = qc(asym, mild, hosp), 
                symptoms   = qc(mild, hosp))
  ...
  # Initiate list for collecting key epidemiological statistics
  ...
  for (i in 1 : p$n_days) {
    ...
    # Report progress by printing day (useful when running on the cluster)
    if (verbose == "day") 
    ...
    list[m, epi_stats] = fn_epi_stats(m, p, ppl, epi_stats, states, i)
    ...
    # Non-RSV-related 'natural' deaths
    list[m, ppl] = fn_natural_death(m, p, ppl, i)
    
    # Replace deceased (RSV or otherwise) with newborns
  ...
  results = format_output(m, p, network, ppl, yaml, seed)
...
create_ppl = function(p, n = NULL, init = TRUE, verbose = "none") {
  ...
  # All people are initiated as female - time to assign males
  ppl[sample_vec(1 : n, size = n * p$gender_split$male), gender := "male"]
  
  # Loop through genders
  for (this_gender in p$genders) {
    
    # Demography pyramid for this gender and number to assign
    gender_demog = p$demography[[this_gender]]
    gender_pop   = ppl[gender == this_gender, .N]
    
    # Sample ages from some distribution and a birthday index
    sample_ages = sample_vec(x = p$ages,  size = gender_pop, replace = TRUE, prob = gender_demog)
    sample_bday = sample_vec(x = 0 : 364, size = gender_pop, replace = TRUE)
    
    # Apply age (use zero for newborns)
    if (init == TRUE)  ppl[gender == this_gender, age := sample_ages]
    if (init == FALSE) ppl[gender == this_gender, age := 0]
    
    # Apply birthday index - do not use 0 for newborns to stagger ageing
    ppl[gender == this_gender, birthday := sample_bday]
  }
...
initiate_epidemic = function(p, ppl) {
  ...
  # 
  # # Assign first booster delivery day, bounded below if force-starting
  # booster_df[, booster_due := pmin(pmax(cycle_period - days_vaccinated, start), force_start, na.rm = TRUE)]
  # 
  # # Count the number of boosters received up until initialise day
  # booster_df[booster_due < 0, n_booster_doses := -booster_due %/% cycle_period]
  # 
  # # If first booster is due in the past, determine most recent dose and assign day of next dose 
...
preallocate_output = function(p) {
  ...
  m = rbindlist(m) %>%
    ...
    arrange(metric, grouping, day, group)
...
# ---------------------------------------------------------
update_output = function(m, p, day_idx, update_metric, to_count, denom = 1) {
  ...
  for (this_grouping in metric_groupings) {
    ...
    if (this_grouping == "na") {
      ...
      m[metric == update_metric & day == day_idx & is.na(group), value := metric_count]
  ...
  # Check the update: m[metric == update_metric & day == day_idx, ]
...
# Prepare final model output
# ---------------------------------------------------------
format_output = function(m, p, network, ppl, yaml, seed) {
  ...
  # m[metric == "variant_prevalence", value := 100 * value / max(sum(value), 1), by = "day"]
  ...
  # Determine whether network should be stored (if 'all' or if 'baseline' and running baseline)
  save_network = o$save_network == "all" || (o$save_network == "baseline" && p$.id == "baseline")
  
  # Create trivial variable if not saving network
  if (!save_network) {
    network_age = NULL 
    
  } else {  # Otherwise append some further details
...
# ---------------------------------------------------------
update_state = function(p, ppl, day_idx, m = NULL, cost_units = NULL) {
  ...
  if (length(update_id) > 0) {
    ...
    update_df = ppl[update_id, ] %>%
      left_join(p$model_flows, by = c("disease_state", "prognosis_state"))
    ...
    if (!is.null(m)) {
      ...
      for (update_metric in unique(na.omit(update_df$metric)))
        m = update_output(m, p, day_idx, update_metric, update_df[metric == update_metric, ])
    }
    
    # Store numbers entering hospital/ICU if required
    if (!is.null(cost_units)) {
      cost_units$hospital[day_idx] = update_df[metric == "hospital_admissions", .N]
      # cost_units$icu[day_idx]      = update_df[metric == "icu_admissions", .N]
    }
    
    # Update disease state according to model_flows
    ...
    if (length(reset_id) > 0) {
      ...
      # ppl[reset_id, diagnosis_day   := NA]
      # ppl[reset_id, treatment_day   := NA]
    ...
    # ---- Initaite infectious period and possible test & treat days ----
    ...
    # 
    # # Day of potential test and diagnosis - see fn_test_diagnose for probability
    # ppl[new_symptoms_id, test_day := day_idx + p$diagnosis_delay]
    
    # The above chunk is appropriate if we have a pre-symptomatic phase, otherwise...
    
    # Newly infectious people are those going FROM the latent phase
    new_infectious_id = update_df[disease_state == "latent", id]
    
    # Signal start of infectious period by starting 'days infectious' count
    ppl[new_infectious_id, days_infectious := 0]  # Incremented at end of daily loop
    
    # # Newly symptomatic people are those going INTO the mild phase
    # new_symptoms_id = update_df[next_disease == "mild", id]
    # 
    # # Day of potential test and diagnosis - see fn_test_diagnose for probability
    # ppl[new_symptoms_id, test_day := day_idx + p$diagnosis_delay]
  }
...
# Calculate basic epidemiological indicators
# ---------------------------------------------------------
fn_epi_stats = function(m, p, ppl, epi_stats, states, day_idx) {
  
  # Total number of people currently infected and infectious
  epi_stats$all_infected   = ppl[disease_state %in% states$infected,   id]
  epi_stats$all_infectious = ppl[disease_state %in% states$infectious, id]
  
  # Easy access total number of people currently infected
  n_infected = length(epi_stats$all_infected)
  
  # Store total number of people infected and infectious
  m = update_output(m, p, day_idx, "currently_infected",   ppl[epi_stats$all_infected, ])
  m = update_output(m, p, day_idx, "currently_infectious", ppl[epi_stats$all_infectious, ])
  
  # Total number of people currently in isolation
  # currently_isolated = ppl[care_state %in% "iso", id]
  # m = update_output(m, p, day_idx, "currently_isolated", ppl[currently_isolated, ])
  
  # Total number of people currently infected with symptoms (mild, severe, or critical)
  currently_symptomatic = ppl[disease_state %in% states$symptoms, id]
  m = update_output(m, p, day_idx, "currently_symptomatic", ppl[currently_symptomatic, ])
  ...
  m = update_output(m, p, day_idx, "pop_susceptibility", pop_susceptibility)
  
  # Prevalence across whole population (proportion currently infected, in %)
  pop_prevalence = 100 * n_infected / p$population_size
  m = update_output(m, p, day_idx, "pop_prevalence", pop_prevalence)
  
  # Sero-prevalence across whole population (proportion that have been infected thus far, in %)
  pop_seroprevalence = 100 * ppl[num_infections > 0, .N] / p$population_size
  m = update_output(m, p, day_idx, "seroprevalence", pop_seroprevalence)
  
  # Virus variant of each infected individual
  # all_variants = factor(ppl[all_infected, variant], levels = p$variants$id)
  # m = update_output(m, p, day_idx, "variant_prevalence", ppl[all_infected, ])
...
# ---------------------------------------------------------
fn_epi_update = function(m, p, ppl, day_idx, cost_units, states) {
  ...
  m = update_output(m, p, day_idx, "hospital_beds", ppl[disease_state == "hosp", ])
  # m = update_output(m, p, day_idx, "icu_beds",      ppl[disease_state == "icu", ])
  
  # Update disease and care state then sample duration of next phase
  list[ppl, m, cost_units] = update_state(p, ppl, day_idx, m = m, cost_units = cost_units)
  ...
  for (state in p$model_states$disease)
    states[[state]][day_idx] = length(ppl[disease_state == state, id])
...
# ---------------------------------------------------------
fn_transmission = function(m, p, ppl, network, epi_stats, day_idx) {
  
  # ---- Sample contacts considering NPIs ----
  
  # NPI effect on at this time step
  npi_effect = p$npi_contact_reduction[day_idx]
  ...
  if (npi_effect > 1e-6) {
    ...
    exposures = network[effective_idx][from %in% epi_stats$all_infectious & 
                                         to %in% epi_stats$all_susceptible]
    
  } else {  # No sampling needed if there is no NPI effect
    exposures = network[from %in% epi_stats$all_infectious & 
                          to %in% epi_stats$all_susceptible]
  }
  ...
  list[ppl, exposures] = fn_immunity_infection(p, ppl, day_idx, exposures) #, epi_stats$variant_prevalence
  ...
  transmission_df = exposures %>% 
    ...
    mutate(probability = pmin(beta * (1 - immunity), 1), 
           rand = runif(n()), 
    ...
    ungroup() %>%
    # Variant is passed down from infector...
    # mutate(variant = ppl[from, variant]) %>%
    as.data.table()
  ...
  m = update_output(m, p, day_idx, "new_local_infections", ppl[local_id, ])
...
# Import infections acquired elsewhere
# ---------------------------------------------------------
fn_importation = function(m, p, ppl, epi_stats, id, day_idx) {
  ...
  m = update_output(m, p, day_idx, "new_importations", ppl[id$import, ])
...
# New viral variant emergence
# ---------------------------------------------------------
fn_emergence = function(m, p, ppl, epi_stats, id, day_idx) {
  ...
  if (day_idx %in% p$variants$import_day) {
    
    # Details of the variant to be imported
    this_variant = p$variants[p$variants$import_day == day_idx, ]
...
# Count incidence and serial intervals for all new infections
# ---------------------------------------------------------
fn_incidence = function(m, p, ppl, epi_stats, id, day_idx) {
  ...
  m = update_output(m, p, day_idx, "all_new_infections", ppl[all_new_infections, ])
  
  # Store incidence in this time step (including imported cases)
  epi_stats$incidence[day_idx] = length(new_infections)
  ...
  # epi_stats$serial_int[[day_idx]] = ppl[id %in% infector_id, days_infected]
...
fn_begin_infection = function(p, ppl, id) { #, apply_variant) {
  ...
  ppl[id, days_next_event := pmax(1, round(p$duration$latency_phase(id)))]
...
fn_prognosis = function(p, new_infected) {
  ...
  prognosis_df = new_infected %>% 
    ...
    left_join(p$prognosis$age, by = "age",  # Age-related prognosis 
...
# ---------------------------------------------------------
fn_test_diagnose = function(m, p, ppl, day_idx) {
  ...
  test_potential   = ppl[test_day == day_idx & !is.na(days_infectious), ]
  ...
  if (n_test_potential > 0) {
    ...
    # Assign the day that this diagnosis is to occur
    ppl[dx_id, diagnosis_day := day_idx]
  }
  ...
  if (day_idx %in% p$testing$mass_testing$when) {
    
    # Number of people to mass test on this day
    #
    ...
    mass_diagnose_id = setdiff(mass_diagnose_id, ppl[diagnosis_day <= day_idx, id])
    
    # Assign diagnosis day and remove future test day
    ppl[mass_diagnose_id, diagnosis_day := day_idx]
    ppl[mass_diagnose_id, test_day := NA]
  }
  
  # Remove reference to all expired test days
  ppl[test_day <= day_idx, test_day := NA]
  
  # Store total number of those diagnosed this time step
  m = update_output(m, p, day_idx, "confirmed", ppl[diagnosis_day == day_idx, ])
...
# Isolate newly diagnosed individuals where appropriate
# ---------------------------------------------------------
fn_isolate = function(m, p, ppl, day_idx) {
  ...
  can_isolate_id = ppl[diagnosis_day == day_idx & 
...
# ---------------------------------------------------------
fn_vaccinate = function(m, p, ppl, day_idx, n_priority_group, cost_units) {
  
  # Extract vaccine type currently being distributed
  current_vaccine = p$vaccine_update %>%
    filter(release_day <= day_idx) %>%
  ...
  coverage_diff   = coverage_matrix[day_idx : (day_idx + 1), ] %>%
  ...
  for (group in p$priority_groups$id) {
    ...
    if (diff(coverage_diff[, group]) > 0) {
      ...
      n_desired = round(p$vaccine$coverage[day_idx, group] * n_group)
  ...
  # Set day of first booster dose (for those recieving final dose of initial schedule today)
  booster_1_df = ppl %>%
    ...
    left_join(y  = p$booster_details[, -"probability"], 
              by = c("priority_group" = "id")) %>%
    mutate(booster_due = pmin(pmax(cycle_period + day_idx, start), 
                              force_start, na.rm = TRUE), 
           booster_due = ifelse(booster_due <= force_end, booster_due, NA))
  
  # Set day of subsequent booster dose (for those receiving a booster today)
  booster_n_df = ppl %>%
  ...
  # Apply next booster due day calculated above
  ...
  list[m, cost_units] = fn_vaccine_count(m, p, ppl, day_idx, cost_units)
...
# ---------------------------------------------------------
fn_vaccine_count = function(m, p, ppl, day_idx, cost_units) {
  ...
  m = update_output(m, p, day_idx, "n_vaccinated",     vaccine_today)
  m = update_output(m, p, day_idx, "total_vaccinated", vaccine_total)
  ...
  m = update_output(m, p, day_idx, "n_doses",     all_doses)
  m = update_output(m, p, day_idx, "total_doses", all_doses)  # Cum-summed in format_output
  ...
  m = update_output(m, p, day_idx, "vaccine_coverage",     vaccine_total, denom = vaccine_elig)
  m = update_output(m, p, day_idx, "vaccine_coverage_pop", vaccine_total, denom = ppl)
  ...
  m = update_output(m, p, day_idx, "booster_coverage",     booster_total, denom = vaccine_total)
  m = update_output(m, p, day_idx, "booster_coverage_pop", booster_total, denom = ppl)
  
  # Boosted in past 365 days
  booster_12m = ppl[days_booster >= 0 & days_booster <= 365, ]
  
  # Coverage of boosters in past 365 days
  m = update_output(m, p, day_idx, "booster_coverage_12m",     booster_12m, denom = vaccine_total)
  m = update_output(m, p, day_idx, "booster_coverage_12m_pop", booster_12m, denom = ppl)
  
  # Update number of vaccine doses in intervention unit counter
  cost_units$vaccine[day_idx] = all_doses[, .N]
...
# Pre-exposure prophylaxis for those eligible
# ---------------------------------------------------------
fn_prep = function(m, p, ppl, day_idx, cost_units) {
  ...
  n_desired = round(p$prep$coverage[day_idx] * n_total)
  ...
  m = update_output(m, p, day_idx, "n_prep",     prep_today)
  m = update_output(m, p, day_idx, "total_prep", prep_total)
  ...
  m = update_output(m, p, day_idx, "prep_coverage",     prep_total, denom = prep_elig)
  m = update_output(m, p, day_idx, "prep_coverage_pop", prep_total, denom = ppl)
  
  # Update number of vaccine doses in intervention unit counter
  cost_units$prep[day_idx] = prep_today[, .N]
...
# Treat eligible infected cases
# ---------------------------------------------------------
fn_treatment = function(m, p, ppl, day_idx, cost_units) {
  ...
  treat_elig = ppl %>%
    ...
    filter(treat_prob > 0, 
           available <= day_idx,
           diagnosis_day == day_idx - p$treat_delay)
  ...
  # Assign current day index to treatment_day for only those successful
  ppl[success_id, treatment_day := day_idx]
  ...
  m = update_output(m, p, day_idx, "n_treat",     ppl[total_id, ])
  m = update_output(m, p, day_idx, "total_treat", ppl[total_id, ]) # Cum-summed in format_output
  
  # Also store the subset that had successful treatment
  m = update_output(m, p, day_idx, "n_treat_success",     ppl[success_id, ])
  m = update_output(m, p, day_idx, "total_treat_success", ppl[success_id, ]) # Cum-summed in format_output
  
  # Update number of vaccine doses in intervention unit counter
  cost_units$treatment[day_idx] = length(total_id)
...
# ---------------------------------------------------------
fn_immunity_infection = function(p, ppl, day_idx = 0, exposures = NULL) { #,variant_prevalence
  ...
  if (!is.null(exposures)) {
    ...
    # NOTE: At the moment maternal immunity is pretty basic: every child gets a predefined amount of 
    #       immunity based purely on how long ago they were born. An imporvement would be to look at 
    #       population level seroprevalence at a given time and scale immunity based on this. To go 
    #       further, could link each birth with a mother and deduce maternal immunity from her infection
    #       status/history.
    
    # Maternal immunity effect
    maternal_df = exposures %>%
      select(id = to) %>%  # Only need to consider the susceptible indivdual
      distinct() %>%  # And only need to consider them once
      left_join(y  = ppl[, .(id, age, birthday)],
                by = "id") %>%
      mutate(days_since_birth = (age + 1) * 365 - birthday + day_idx) %>%
      filter(days_since_birth <= length(p$maternal_immunity)) %>%
      mutate(maternal_effect = p$maternal_immunity[pmax(days_since_birth, 1)]) %>%
      select(id, maternal_effect)
    
    # Aquired immunity effect
    acquired_df = exposures %>%
      select(id = to) %>%  # Only need to consider the susceptible indivdual
      distinct() %>%  # And only need to consider them once
      left_join(y  = ppl[, .(id, days_recovered)],
                by = "id") %>%
      filter(!is.na(days_recovered)) %>%
      mutate(acquired_effect = p$acquired_immunity[pmax(days_recovered, 1)]) %>%
      select(id, acquired_effect)
    
    # Consider immune escape for all immunity elements
    escape_df = exposures %>%
      # left_join(y  = ppl[, .(id, from_variant = variant)],
      #           by = c("from" = "id")) %>%  # Variant of infectious individual
      # left_join(y  = ppl[, .(id, to_variant = variant, vaccine_type, vaccine_effect)],
      #           by = c("to" = "id")) %>%  # Vaccine and previous exposure details of susceptible
      left_join(y  = maternal_df, 
                by = c("to" = "id")) %>%  # Maternal immunity of susceptible
      left_join(y  = acquired_df,
                by = c("to" = "id")) %>%  # Acquired immunity of susceptible
      # Immuno escape effect - depends of previous exposure and vaccine type...
      ...
      mutate(escape = 1, # 1 - p$variants$immuno_escape[match(from_variant, p$variants$id)],
             escape_vaccine = 1, # ifelse(target_of_vaccine == TRUE,  1, escape),
             escape_other   = 1) %>% # ifelse(from_variant == to_variant, 1, escape) %>%
      mutate(immunity_acquired = pmax(acquired_effect * escape_other, 0, na.rm = TRUE), 
             immunity_maternal = pmax(maternal_effect * escape_other, 0, na.rm = TRUE)) %>%
             # immunity_vaccine  = vaccine_effect  * escape_vaccine * p$vaccine$infection_blocking)) %>%
      select(starts_with("immunity"))
    
    # Calculate hybrid immunity for all possible methods
    #
    # NOTE: Whilst this looks pretty, it's super slow
    # hybrid_df = escape_df %>%
    #   rowwise() %>% 
    #   mutate(sum  = sum(c_across(starts_with("immunity"))), 
    ...
    # Calculate hybrid immunity for all possible methods
    #
    # NOTE: Uglier and more cumbersome than above syntax, but much faster
    hybrid_df = escape_df %>%
  ...
  # TODO: Consider maternal immunity here too...
  ...
  # Without variants and vaccinations, the above chunk can be reduced to...
  
  # Calculate effect of acquired immunity
  ppl[, immune_state := pmax(p$acquired_immunity[pmax(days_recovered, 1)], 0, na.rm = TRUE)]
...
# Non-RSV-related 'natural' deaths
# ---------------------------------------------------------
fn_natural_death = function(m, p, ppl, day_idx) {
  
  # Check natural death flag
  if (p$natural_deaths == TRUE) {
    
    # IDs of all those that die naturally, regardless of RSV status
    ...
    m = update_output(m, p, day_idx, "natural_deaths", natural_deaths)
  }
...
# ---------------------------------------------------------
fn_birth = function(m, p, ppl, day_idx) {
  ...
  if (day_idx > 1)
    m = update_output(m, p, day_idx, "births", length(birth_id))
...
# ---------------------------------------------------------
fn_ageing = function(m, p, ppl, day_idx) {
  ...
  if (p$annual_ageing == TRUE) {
    ...
    day_of_year = day_idx %% 365  # Day index 0 used instead of 365
    ...
    ppl[age_idx, age := pmin(age + 1, p$age_max - 1)]
  }
  ...
  m = update_output(m, p, day_idx, "pop_age", pop_age)
